...
//       5.schema校验 6.generateRoot 内联 :root 7.审核门
//
// Usage: const { runPipeline } = require('./growth/growth-agent');
...
const { generateRoot } = require(path.join(PROJECT_DIR, 'scripts', 'tokens-to-css.cjs'));
...
function assembleTokens(mapping, siteData, slug) {
  ...
  const colorTokens = [
    { name: '--color-primary', $type: 'color', value: mapping['--color-primary'], role: 'accent', description: '主强调色' },
    { name: '--color-secondary', $type: 'color', value: mapping['--color-secondary'], role: 'accent', description: '辅强调色' },
    { name: '--color-surface', $type: 'color', value: mapping['--color-surface'], role: 'surface-bg', description: '页面背景色' },
    { name: '--color-surface-container-low', $type: 'color', value: mapping['--color-surface-container-low'], role: 'surface-card', description: '卡片/面板背景' },
    { name: '--color-on-surface', $type: 'color', value: mapping['--color-on-surface'], role: 'text-primary', description: '主文字色' },
    { name: '--color-on-surface-variant', $type: 'color', value: mapping['--color-on-surface-variant'], role: 'text-secondary', description: '次级文字色' },
    { name: '--color-outline', $type: 'color', value: mapping['--color-outline'], role: 'border-default', description: '边框色' },
    { name: '--line-soft', $type: 'color', value: mapping['--line-soft'], role: 'border', description: '浅边框色' },
  ].filter(t => t.value);

  const typographyTokens = [
    { name: '--font-display', $type: 'fontFamily', value: mapping['--font-display'], role: 'display-font', description: '标题字体栈' },
    { name: '--font-body', $type: 'fontFamily', value: mapping['--font-body'], role: 'body-font', description: '正文字体栈' },
    { name: '--font-mono', $type: 'fontFamily', value: mapping['--font-mono'], role: 'mono-font', description: '等宽字体栈' },
  ].filter(t => t.value);

  const spacingTokens = [
    { name: '--page-wmax', $type: 'dimension', value: mapping['--page-wmax'], role: 'content-max-width', description: '内容最大宽度' },
    { name: '--page-pad', $type: 'dimension', value: mapping['--page-pad'], role: 'page-padding', description: '页面水平内边距' },
    { name: '--gap', $type: 'dimension', value: mapping['--gap'], role: 'element-gap', description: '元素/网格间距' },
    { name: '--gutter', $type: 'dimension', value: mapping['--gutter'], role: 'column-gutter', description: '列间距/留白基准' },
  ].filter(t => t.value);

  const radiusTokens = [
    { name: '--radius', $type: 'dimension', value: mapping['--radius'], role: 'radius', description: '圆角基准值' },
  ].filter(t => t.value);

  const shadowTokens = [
    { name: '--shadow-sm', $type: 'shadow', value: mapping['--shadow-sm'], role: 'shadow', description: '轻阴影' },
    { name: '--shadow-md', $type: 'shadow', value: mapping['--shadow-md'], role: 'shadow', description: '中阴影' },
  ].filter(t => t.value);

  const motionTokens = [
    { name: '--ease-default', $type: 'raw', value: mapping['--ease-default'], role: 'easing', description: '标准缓动曲线' },
    { name: '--duration-base', $type: 'duration', value: mapping['--duration-base'], role: 'duration', description: '基础过渡时长' },
  ].filter(t => t.value);
...
function writeAndSync(tokensData, slug) {
  ...
  // Generate :root from tokens.json (replaces sync-roots.js)
  const rootCSS = generateRoot(tokensData.tokens);
  let html = fs.readFileSync(tmplPath, 'utf-8');

  const rootStart = html.indexOf(':root');
  if (rootStart === -1) {
    return { ok: false, error: ':root 生成失败 — template.html 无 :root 块' };
  }
  const braceStart = html.indexOf('{', rootStart);
  if (braceStart === -1) {
    return { ok: false, error: ':root 生成失败 — template.html :root 无 { 块' };
  }
  let depth = 0;
  let rootEnd = -1;
  ...
  if (rootEnd === -1) {
    return { ok: false, error: ':root 生成失败 — template.html :root 块未闭合' };
  }
  html = html.slice(0, rootStart) + rootCSS + html.slice(rootEnd);
  fs.writeFileSync(tmplPath, html, 'utf-8');

  // Verify :root was written
  if (!rootCSS.includes('--')) {
    return { ok: false, error: ':root 生成验证失败 — 无任何 token 变量' };
  }
...
visual_family: 选 editorial | swiss | brutalist | retro | organic | graphic 之一（模板长什么样）
content_type: 选 landing | portfolio | blog | corporate | dashboard | ecommerce | personal 之一（页面用途）
tone: 选 1-3 个，从 dramatic | playful | warm | confident | sober | friendly | upbeat | punchy（情绪调性）`;
}

async function generateTemplateContent(siteData, tokensData, hostname, skeletonPath) {
  // skeleton 已移除（design.md 驱动渲染）；placeholders 在 prompt 构建中未使用
  const placeholders = skeletonPath ? extractPlaceholders(skeletonPath) : new Set();
  ...
  try {
    ...
    const visual_family = parsed.visual_family || null;
    const content_type = parsed.content_type || null;
    const tone = parsed.tone || [];
    delete parsed.visual_family;
    delete parsed.content_type;
    delete parsed.tone;
    return { ok: true, content: parsed, visual_family, content_type, tone };
  } catch (e) {
...
async function runPipeline(inputUrl, callbacks = {}) {
  ...
  try {
    ...
    // Step 5: Write + sync
    emit(5, 'running', { message: '写入文件 + generateRoot :root 生成...' });
    ...
    let contentResult = { ok: false, content: null, visual_family: null, content_type: null, tone: [] };
    try {
      ...
      if (contentResult.ok) {
        emit(6, 'done', {
          ...
          visual_family: contentResult.visual_family,
    ...
    // Step 7: Write pipeline metadata for approve endpoint
    // template.html 已由 Step 5 writeAndSync 生成（tokens.json → :root）
    try {
      emit(7, 'running', { message: '写入管线元数据...' });
      ...
      emit(7, 'done', { message: '元数据已写入', contentGenerated: contentResult.ok });
    } catch (metaErr) {
      emit(7, 'done', { warning: '元数据写入失败: ' + metaErr.message });
    }
