 ...
 *   2. registry.json 每个条目 visual_family/content_type/tone 字段在枚举内
...
for (const entry of registry) {
  ...
  if (!entry.visual_family) { fail(entry.slug + ': missing visual_family'); }
  else if (!vfValid.has(entry.visual_family)) { fail(entry.slug + ': invalid visual_family=' + entry.visual_family); }

  if (!entry.content_type) { fail(entry.slug + ': missing content_type'); }
  else if (!ctValid.has(entry.content_type)) { fail(entry.slug + ': invalid content_type=' + entry.content_type); }

  if (!entry.tone || entry.tone.length === 0) { fail(entry.slug + ': missing tone'); }
  else {
    for (const t of entry.tone) {
      if (!toneValid.has(t)) { fail(entry.slug + ': invalid tone=' + t); }
    }
  }
}
ok(total + ' entries checked (placeholder skipped)');
...
if (errors > 0) {
  ...
  console.log('All checks PASS. ' + total + ' templates labeled.');
  process.exit(0);
}
